	...
	return (
		<CometCard translateDepth={4} rotateDepth={8} glareOpacity={0.15}>
			<div
				{...props}
				className={cn(
					"group relative flex aspect-page size-full cursor-pointer overflow-hidden rounded-2xl border border-white/20 dark:border-white/10 bg-background/40 p-1 shadow-lg backdrop-blur-xl transition-all duration-300 hover:scale-[1.02] hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/20",
					className,
				)}
			>
				...
				<div className="relative flex size-full flex-col overflow-hidden rounded-xl bg-card/40 backdrop-blur-md">
					...
					{/* Bottom Glass Pill */}
					<div className="absolute inset-x-0 bottom-0 flex w-full flex-col justify-end gap-y-1 border-t border-white/10 bg-background/70 px-4 py-3.5 backdrop-blur-xl transition-colors duration-300 group-hover:bg-background/85">
