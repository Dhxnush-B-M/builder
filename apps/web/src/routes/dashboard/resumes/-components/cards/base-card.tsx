	...
	return (
		<CometCard translateDepth={6} rotateDepth={10} glareOpacity={0.25}>
			...
			>
				...
				<div
					aria-hidden="true"
					className="pointer-events-none absolute -top-12 -right-12 size-40 rounded-full bg-gradient-to-br from-emerald-500/20 via-teal-500/10 to-transparent opacity-50 blur-2xl transition-opacity duration-500 group-hover:opacity-100"
				/>
				...
				<div className="relative flex size-full flex-col overflow-hidden rounded-2xl bg-card/30 backdrop-blur-md">
					...
					{/* Bottom Glass Pill */}
					<div className="absolute inset-x-0 bottom-0 flex w-full flex-col justify-end gap-y-1.5 border-white/15 border-t bg-background/80 px-4 py-4 backdrop-blur-2xl transition-all duration-300 group-hover:bg-background/95">
						...
						<div className={cn("mt-1.5 hidden flex-wrap items-center gap-1", tags && tags.length > 0 && "flex")}>
							{tags?.map((tag) => (
								<Badge
									key={tag}
									variant="secondary"
									className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 font-semibold text-[10px] text-emerald-400 shadow-sm"
								>
									{tag}
								</Badge>
							))}
