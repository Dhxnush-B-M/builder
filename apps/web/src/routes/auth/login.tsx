...
function AuthLoginPage() {
	...
	// Listen for direct Google OAuth 2.0 callback
	useEffect(() => {
		...
		if (accessToken) {
			// Wipe hash immediately to keep address bar clean
			...
			fetch("https://www.googleapis.com/oauth2/v3/userinfo", {
				...
				.then(async (googleUser) => {
					if (googleUser?.email) {
						...
						const realAvatar = googleUser.picture || "";
	...
	async function handleSubmit(e: FormEvent) {
		...
		try {
			...
			localStorage.setItem(
				"rbuilder_user",
				JSON.stringify({ email: typedEmail, name: typedName }),
			);
			...
			void saveUserToSupabase({ email: typedEmail, name: typedName }).catch(() => null);
			void navigate({ to: "/dashboard/resumes" });
		} catch (err) {
