...
import { DownloadSimpleIcon, PlusIcon, ReadCvLogoIcon } from "@phosphor-icons/react";
...
import { useLocalResumes } from "@/libs/resume/local-storage";
...
function RouteComponent() {
	...
	const filteredResumes = useMemo(() => {
		const list = [...localResumes, ...(resumes ?? [])];
		...
		return all.filter(
			(resume) =>
				(resume.name || "").toLowerCase().includes(query) || (resume.slug || "").toLowerCase().includes(query),
		);
